...
# Write out unmerged bed of whole alignments
#target_microsat %>%
#  dplyr::select(contig, aln_start , aln_end, class, motif_n, seqnames ) %>%
#  write_tsv("target_microsats.bed", col_names = NA)
#
...
# Need to merge the start and end as well
...
#bamlist=$(find $(/usr/bin/ls -d /group/pathogens/Alexp/phylloxera/bams) -maxdepth 1 | grep ".bam$" | sort | uniq | paste -s -d ',')
...
#
## Get list of clipped reads (those with H or S in column 6)
#samtools view merged_filtered.bam | awk '$6 ~ /H|S/{print $1}' | sort -k1,1 | uniq > names.txt
## sort original bam on names:
#samtools view merged_filtered.bam | sort -k1,1 > tmp.sam
## get header
#samtools view -H merged_filtered.bam > merged_filtered_noclipped.sam
## outer-join (I use ctrl+v+tab to explicitly get the tab character)
#join -t '   ' -v 1 -1 1 -2 1 tmp.sam names.txt >> merged_filtered_noclipped.sam
## convert to bam
#samtools view -bS merged_filtered_noclipped.sam > merged_filtered_noclipped.bam
## index bam
#samtools index merged_filtered_noclipped.bam
...
#rm merged_filtered_noclipped.sam
...
while read r; do
  ...
  for i in $(bcftools query -l tmp.vcf.gz );	do 
      ...
      #else
        #echo -e ">${i}_${outname}_1_\n" >> ${outname}.fa
        #echo -e ">${i}_${outname}_2_\n" >> ${outname}.fa
      fi
  done
  # Align outputs
...
r=$(sed -n '1p' < /group/pathogens/IAWS/Projects/Phylloxera/microsat/microsats.bed)
...
# HGMFWDSXY_Y21S0015-dv18g4vaitc8152 for contig_4038 is good test
...
while read r; do
  ...
  for i in $(bcftools query -l tmp.vcf.gz );	do 
      ...
      # Subset the GATK vcf 
      #bcftools view gatk_calls.vcf.gz -s ${i} -Oz | \
      #bcftools filter -i 'FILTER ="PASS"' -t $(cat tmp.regions) -Oz -o gatk_tmp.vcf.gz
      # SHould i keep some filters-  maybe quality?
...
for (i in 1:length(ref_alleles)){
  ...
  if(length(genotyped_seqs) > 0) {
    ...
    names(genotyped_seqs) <- paste0(vaitc, "_", genotype, "_",haplotype)
...
# Replace this with those from the reference fastas? 
# Output alignments?

#exp_length <- readxl::read_xlsx("doc/PhylloxeraGenotypes_DistanceTree.xlsx", #sheet="Data", skip = 2) %>%
#  magrittr::set_colnames(c("genotype","status", "Dvit4_1", "DVit4_2", "Dvit6_1", #"Dvit6_2", "Dvit1_1", "Dvit1_2", "Dvit5_1", "Dvit5_2", "Dvit5_2","Dvit3_1", #"Dvit3_2", "Dvit2_1", "Dvit2_2")) %>%
#  pivot_longer(-c("genotype", "status"),
#               names_to="locus",
#               values_to="exp_length") %>%
#  mutate(genotype = str_replace(genotype, "G0", "G"))%>%
#  mutate(strand = locus %>% str_remove("^.*_")%>% as.numeric(),
#         locus = locus %>% str_remove("_.*$"))

exp_length <- readxl::read_xlsx("doc/AlleleSizes.xlsx",range = "A11:Q18" ) %>%
  dplyr::rename(genotype = Fluoroscent) %>%
...
seqs <- fs::dir_ls("combined_fasta/", glob="*.fa") %>%
  purrr::map(function(x){
    if (file.size(x) > 0){
      insect::readFASTA(x)
    } else {
...
seqs <- seqs[str_detect(names(seqs), "vaitc|Australia")]
seqs <- ape::del.gaps(seqs)

obs_length <- enframe(names(seqs), value="name", name=NULL) %>%
  mutate(name = name %>% str_replace("Australia_", "Australia-SRR10418414gA7_"),
         contig = str_remove(name, "^.*contig_"),
         name = name %>%
            str_remove("_contig.*$") %>%
            str_remove("^.*-"),
         locus = name %>% str_remove("^.*?_"),
         name = name %>% 
           str_remove("_.*$") %>%
           str_remove("vaitc.*$"),
         sample_id = name %>% str_remove("g.*$"),
         genotype = name %>% str_remove(sample_id) %>% toupper(),
         strand = locus %>% str_remove("^.*_") %>% as.numeric(),
         locus = locus %>% str_remove("_.*$")) %>%
  dplyr::select(-name) %>%
...
formatted_sizes <- joint %>%
  ...
  arrange(match(genotype, c("G1", "G4", "G7","G19", "G20","G30", "G38", "GA7")))

write_csv(formatted_sizes, "output/formatted_sizes_new.csv")
```


# Align against reference alleles
...
# Read in fastas and get size
seqs <- fs::dir_ls("gatk_fasta/", glob="*.fa") %>%
  purrr::map(function(x){
    if (file.size(x) > 0){
      insect::readFASTA(x)
    } else {
...
write_csv(formatted_sizes, "output/formatted_sizes_gatk.csv")
```
