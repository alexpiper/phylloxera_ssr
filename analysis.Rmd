...
microsat_primers %>% 
  ...
  write_tsv("output/primers.txt", col_names = NA)
...
microsat_primers %>% 
 ...
 #        end_pos = end_pos+100) %>%
  write_tsv("output/microsats.bed", col_names = NA)
...
target_microsat %>%
  ...
  write_tsv("output/target_microsats.bed", col_names = NA)
...
rng %>%
  ...
  dplyr::select(contig, start, end, class, motif_n, seqnames ) %>%
  write_tsv("output/target_microsats_merged.bed", col_names = NA)
...
```{r}
flank_bed <- read_table("output/trf_ssr_edited.bed", col_names = c("contig", "start", "end", "class", "motif_n", "name", "motif" )) %>%
...
flank_bed %>% 
  write_tsv("output/flank_sequences.bed", col_names = NA)
...
# Write out plot
pdf(file="figs/hipstr_filt.pdf", width = 11, height = 8 , paper="a4r")
  plot(gg.hipstr_filt)
...
refseq_regions <- readDNAStringSet("output/microsats.fa")

# read in just microsattelite region from reference genome
refseq_ssrs  <- readDNAStringSet("output/final_trf_strs.fa")
...
for (i in 1:length(ref_alleles)){
  ...
  if(length(genotyped_seqs) > 0) {
    ...
    BrowseSeqs(aln, 
              ...
              htmlFile=paste0("output/alignments/html/final_",ref_alleles[i],".html"),
              openURL = FALSE)
...
# Write out plot
pdf(file="figs/allele_diffs.pdf", width = 11, height = 8 , paper="a4r")
  plot(gg.allele_diffs)
try(dev.off(), silent=TRUE)
...
write_csv(formatted_sizes, "output/formatted_sizes.csv")
```
